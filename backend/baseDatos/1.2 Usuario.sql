...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM usuario WHERE usuario_asignado = p_usuario_asignado) THEN
        SIGNAL SQLSTATE '45000' 
...
-- Procedimiento para registrar logout
CREATE PROCEDURE sp_registrar_logout(IN p_id_usuario INT)
BEGIN
    UPDATE inicio_sesion 
    SET fecha_ultima_sesion = NOW() 
    WHERE ID_Usuario = p_id_usuario 
    ORDER BY ID_Inicio_Sesion DESC 
    LIMIT 1;
END //

-- Procedimiento para obtener estado de usuario
...
-- Procedimiento para incrementar actividades de técnico
CREATE PROCEDURE sp_incrementar_actividades_tecnico(IN p_id_tecnico INT)
BEGIN
    UPDATE Tecnico 
...
END //
-- Procedimiento para actualizar usuario asignado
...
-- Procedimiento para obtener técnicos por especialidad
...
-- Procedimiento para obtener usuarios por tipo
...
-- Procedimiento para obtener historial de actividades
CREATE PROCEDURE sp_obtener_historial_actividades(IN p_usuario_id INT)
BEGIN
