...
-- Procedimiento para crear notificación de máquina
...
BEGIN
    INSERT INTO NotificacionMaquinaRecreativa (
        ID_Remitente, ID_Destinatario, ID_Maquina, Tipo, Mensaje
    ) VALUES (
        p_id_remitente, p_id_destinatario, p_id_maquina, p_tipo, p_mensaje
    );
END //

-- Procedimiento para obtener notificaciones por destinatario
CREATE PROCEDURE sp_obtener_notificaciones_por_destinatario(IN p_id_destinatario INT)
BEGIN
...
CREATE PROCEDURE sp_marcar_como_leida(IN p_id_notificacion INT)
BEGIN
...
-- Procedimiento para obtener cantidad de notificaciones no leídas
CREATE PROCEDURE sp_obtener_no_leidas(IN p_id_usuario INT, OUT p_total INT)
BEGIN
...
-- Procedimiento para crear notificación de reporte
...
BEGIN
    INSERT INTO notificaciones (
        ID_Reporte, ID_Usuario, mensaje, fecha_hora, leida
    ) VALUES (
        p_id_reporte, p_id_usuario, p_mensaje, NOW(), 0
    );
END //

-- Procedimiento para obtener notificaciones por usuario
CREATE PROCEDURE sp_obtener_notificaciones_por_usuario(IN p_usuario_id INT)
BEGIN
...
-- Procedimiento para marcar todas las notificaciones como leídas
CREATE PROCEDURE sp_marcar_todas_como_leidas(IN p_usuario_id INT)
BEGIN
    UPDATE notificaciones 
    SET leida = 1 
...
-- Procedimiento para obtener cantidad de notificaciones no leídas
CREATE PROCEDURE sp_obtener_cantidad_no_leidas(IN p_usuario_id INT, OUT p_cantidad INT)
BEGIN
