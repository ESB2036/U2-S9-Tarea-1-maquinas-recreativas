...
BEGIN
    ...
    SET p_id_recaudacion = LAST_INSERT_ID();
END //
...
CREATE PROCEDURE sp_eliminar_recaudacion(IN p_id_recaudacion INT)
BEGIN
...
BEGIN
    ...
    SET p_id_informe = LAST_INSERT_ID();
END //
...
BEGIN
    INSERT INTO informe_detalle (
        ID_Informe, ID_Componente, valor_por_componente
    ) VALUES (p_id_informe, p_id_componente, 10.00);
END //

-- Procedimiento para obtener informe principal
CREATE PROCEDURE sp_obtener_informe_principal(IN p_id_recaudacion INT)
BEGIN
...
CREATE PROCEDURE sp_obtener_componentes_informe(IN p_id_informe INT)
BEGIN
    SELECT c.ID_Componente, c.nombre, c.tipo, id.valor_por_componente
