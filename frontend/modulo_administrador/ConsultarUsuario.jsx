...
// Redirige a la página de edición de usuario.
  const handleActualizarUsuario = (id) => navigate(`/admin/gestion-usuarios/editar-usuario/${id}`);
  // Redirige a la página para cambiar el estado del usuario.
  const handleEditarEstadoUsuario = (id) => navigate(`/admin/gestion-usuarios/editar-estado-usuario/${id}`);
// Elimina al usuario después de una confirmación y actualiza la lista.
  const handleEliminarUsuario = async (id) => {
    if (!window.confirm('¿Está seguro de eliminar este usuario?')) return;
    
    try {
      const res = await fetch(`/api/administrador/usuarios/${id}`, {
        method: 'DELETE',
        headers: {
          'Accept': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      if (data.success) {
        setUsuarios(us => us.filter(u => u.ID_Usuario !== id));
      ...
      alert(err.message); // Muestra el mensaje de error
    }
  };
