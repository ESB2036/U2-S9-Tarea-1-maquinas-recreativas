...
export default function Componentes() {
    ...
    const fetchComponentesEnUso = async (userId, machineId = null) => {
        try {
            ...
            const componentesData = Array.isArray(data.componentes) ? data.componentes : [];
        ...
        const fetchData = async () => {
            try {
                ...
                if (data.success) {
                    ...
                    throw new Error(data.message || 'Error al cargar componentes disponibles');
                }

                const componentesUso = await fetchComponentesEnUso(localUser.ID_Usuario, machine?.ID_Maquina);                setComponentesEnUso(componentesUso);
                setLoading(false);
            } catch (err) {
   ...
   const handleUsarComponente = async (idComponente) => {
    try {
        ...
        const updatedComponentesEnUso = await fetchComponentesEnUso(user.ID_Usuario);
    ...
    try {
        ...
        // Actualizar la lista de componentes en uso
        const updatedComponentesEnUso = await fetchComponentesEnUso(user.ID_Usuario, selectedMachine?.ID_Maquina);
        setComponentesEnUso(updatedComponentesEnUso);
    ...
    return (
        <div className="componentes-container">
            ...
            <div className="dashboard-sections">
                        ...
                        <tbody>
                            {componentes.map(comp => (
                                <tr key={comp.ID_Componente}>
                                    ...
                                    <td>
                                        {!componentesEnUso.some(c => c.ID_Componente === comp.ID_Componente) && (
                                            <button
                ...
                <h3>Componentes en Uso {selectedMachine && `- Máquina: ${selectedMachine.Nombre_Maquina}`}</h3>
                {componentesEnUso.length > 0 ? (
                    <table>
                        ...
                        <tbody>
                           {componentesEnUso.map(comp => (
                                <tr key={`${comp.ID_Componente}-${comp.ID_Maquina || 'no-machine'}`}>
                                    <td>{comp.ID_Componente}</td>
                                    <td>{comp.nombre}</td>
                                    <td>{comp.tipo}</td>
                                    ...
                                    <td>
                                        <button 
                                            onClick={() => handleLiberarComponente(comp.ID_Componente)}
                    ...
                    <p>No hay componentes en uso actualmente {selectedMachine && `para la máquina ${selectedMachine.Nombre_Maquina}`}</p>
                )}
            </div>
            </div>
        </div>
