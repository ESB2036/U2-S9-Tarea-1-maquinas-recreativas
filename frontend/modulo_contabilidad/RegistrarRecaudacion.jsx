 ...
 const [showComponents, setShowComponents] = useState(false);
  ...
  const handleShowComponents = () => {
   ...
   const generateInformeData = async () => {
    ...
    try {
      // Obtener datos de la máquina usando el endpoint correcto
      ...
      // Obtener datos del comercio usando el endpoint correcto
      ...
      const ensambladorRes = await fetch(`/api/usuario/profile/${maquinaData.maquina.ID_Tecnico_Ensamblador}`);
      const comprobadorRes = await fetch(`/api/usuario/profile/${maquinaData.maquina.ID_Tecnico_Comprobador}`);
      ...
      if (maquinaData.maquina.ID_Tecnico_Mantenimiento) {
        const mantenimientoRes = await fetch(`/api/usuario/profile/${maquinaData.maquina.ID_Tecnico_Mantenimiento}`);
  ...
  useEffect(() => {
    ...
    fetch(`/api/contabilidad/maquinas-operativas-por-comercio?ID_Comercio=${comercioSeleccionado.ID_Comercio}`)
      .then(r => {
    ...
    <div className="recaudacion-container">
      ...
      <form onSubmit={handleSubmit} className="recaudacion-form">
        ...
        <div className="form-actions">
          ...
          <button 
          type="button" 
          className={`btn btn-success ${!hasRecaudacion ? 'disabled' : ''}`}
          onClick={generateInformeData}
          disabled={!hasRecaudacion}
          title={!hasRecaudacion ? "Primero registre una recaudación" : ""}
        >
          Levantar Informe de Recaudación
        </button>
        </div>
      </form>
{/* Modal de Informe */}
      <Modal
        ...
        onRequestClose={() => setShowInformeModal(false)}
        contentLabel="Informe de Recaudación"
        className="informe-modal"
        overlayClassName="informe-overlay"
      >
        {informeData && (
          <div className="informe-content">
            ...
            <div className="informe-section">
              ...
              <p><strong>Nombre:</strong> {informeData.nombreComercio}</p>
              <p><strong>Dirección:</strong> {informeData.direccionComercio}</p>
              <p><strong>Teléfono:</strong> {informeData.telefonoComercio}</p>
              <p><strong>Tipo:</strong> {informeData.tipoComercio}</p>
            </div>
            
            <div className="informe-section">
              <h3>Datos de la Máquina</h3>
              <p><strong>Nombre:</strong> {informeData.nombreMaquina}</p>
              <p><strong>Monto Recaudado:</strong> ${informeData.montoTotal}</p>
              <p><strong>Fecha:</strong> {new Date(informeData.fecha).toLocaleString()}</p>
            </div>
                ...
                <table>
                  ...
                  <tfoot>
                    <tr>
                      ...
                      <td><strong>${machineComponents.length * 10}.00</strong></td>
                    </tr>
                  </tfoot>
                </table>
              ) : (
            ...
            <div className="informe-totals">
              <h3>Totales</h3>
              <p><strong>Total recaudado:</strong> ${informeData.montoTotal}</p>
              <p><strong>Total pagos a técnicos:</strong> ${(400 * 2) + (tecnicos.mantenimiento ? 400 : 0)}.00</p>
              <p><strong>Total componentes:</strong> ${machineComponents.length * 10}.00</p>
              <p className="grand-total">
                <strong>Total neto para la empresa:</strong> 
                ${ ( (parseFloat(informeData.montoTotal)) - (400 * 2) - (tecnicos.mantenimiento ? 400 : 0) - (machineComponents.length * 10) ) }.00
              </p>
            </div>
            
            <div className="informe-footer">
              <p><strong>Empresa:</strong> recreasys.s.a</p>
